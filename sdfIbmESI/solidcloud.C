...
#include <sstream> //chit
#include "IFstream.H" //chit
...
{
//	Foam::IFstream ifstream = Foam::IFstream(dictfile); //chit
	Foam::IFstream ifstream(dictfile); //chit
    ...
    m_gravity  = vector(meta.lookup("gravity")); // chit -added vector() to convert
   ...
   try {
    ...
    forAll(solids, i)
    {
        ...
        vector pos = vector(solid.lookup("pos")); //chit added vector() to convert
        if (m_ON_TWOD)
        {
            if (pos.z() !=  0)
    ...
    m_cellenum(CellEnumerator(m_mesh))
{
    ...
    const Foam::dimensionedScalar&          rho = readScalar(transportProperties.lookup("rho")); //chit
    m_rhof = rho.value();
    if (!m_ON_FLUID)
        m_rhof = 0.0;
...
void SolidCloud::interact(scalar time, scalar dt)
{
    ...
    m_ct = 0.0; // chit changed to 0.0 from 0
    m_As = 0.0;
