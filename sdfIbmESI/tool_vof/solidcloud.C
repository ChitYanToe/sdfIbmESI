...
//    Foam::IFstream ifstream = Foam::IFstream(dictfile);
   ...
   try {
    ...
    forAll(solids, i)
    {
        ...
        vector pos = vector(solid.lookup("pos")); //chit
        if (m_ON_TWOD)
        {
...
//        s.setMaterialAndShape(m_libmat["mat"], m_libshape[shp_name]); //chit
